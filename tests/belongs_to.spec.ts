...
test.group('BelongsTo', (group) => {
  ...
  test('createMany', async ({ database }) => {
    ...
    await Promise.all([
      ...
      database.assertCount('account', 2),

      database.assertHas('account', { user_id: accountA!.user.id }, 1),
      database.assertHas('account', { user_id: accountB!.user.id }, 1),

      database.assertHas('user', { id: accountA!.user.id }, 1),
      database.assertHas('user', { id: accountB!.user.id }, 1),
    ])
  })
